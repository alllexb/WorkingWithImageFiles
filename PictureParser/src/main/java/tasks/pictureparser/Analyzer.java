...
public class Analyzer {
    ...
    public int[][] extractFigure() {
        removeFigure();
        if (extract == null) return null;
        int[][] figure = new int[pixels.length][pixels[0].length];
        ...
        for (Pixel pixel: extract) {
            figure[pixel.getX()][pixel.getY()] = 0;
        }
        System.out.println("Figure extracted!");
        return figure;
    }
    ...
    public void Count() {
//        int [][] swap =  Arrays.copyOf(pixels,pixels.length);
        if (pixels == null) return;
        if (isBlack()) {
            picCount = 1;
            return;
...
//        pixels = Arrays.copyOf(swap,swap.length);
    }
}
